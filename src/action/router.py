...
from src.config.settings import ActionConfig, PAConfig
...
class ActionRouter:
    ...
    def _decide(
        ...
        # 3b. PA Pre-Check gate — escalate when PA required and approval unlikely
        #     CMS-0057-F governing rule (72hr urgent / 7-day standard turnaround, Jan 2027)
        if (
            score.pa_required is True
            and score.pa_approval_likelihood is not None
            and score.pa_approval_likelihood < PAConfig.PA_ESCALATE_THRESHOLD
        ):
            return self._escalate_decision(
                claim, score, reason="PA_APPROVAL_RISK", governing_rule="CMS-0057-F"
            )
    ...
    def _escalate_decision(
        ...
        rule_cited = governing_rule or score.predicted_denial_code
        return ActionDecision(
            action_id=str(uuid.uuid4()),
            claim_id=claim.get("claim_id", ""),
            score_id=score.score_id,
            ...
            confidence=score.confidence,
            risk_score=score.risk_score,
            governing_rule_cited=rule_cited,
            correction_applied=None,
...
def _build_escalation_draft(claim: dict, score: ScoringResult) -> str:
    ...
    Layout: BILLING GUIDANCE first so billing staff see the action instruction without
    scrolling past technical scores. RISK ASSESSMENT follows for reviewer verification.
    """
    ...
    return dedent(f"""\
        ...
        RECOMMENDED ACTION
        Review the billing guidance above and verify against clinical documentation.
        Approve or reject before submission. This draft was generated by the autonomous
        scoring agent and requires human sign-off before any submission or correction.
        ─────────────────────────────────────────
